...
app.get('/api/employees', authenticateToken, async (req, res) => {
  try {
    const [results] = await db.query(`
      ...
      FROM employees ORDER BY employee_name ASC
    `);
    res.json(results);
  } catch (err) {
  ...
  try {
    ...
    await db.query(
      `INSERT INTO leaves (employee_id, leave_date, leave_type)
       VALUES ? ON DUPLICATE KEY UPDATE leave_type = VALUES(leave_type)`,
      [values]
    );
...
app.get('/api/leaves/by-date', authenticateToken, async (req, res) => {
  ...
  try {
    const [results] = await db.query(`
      ...
      WHERE l.leave_date = ? ORDER BY e.employee_name ASC
    `, [date]);
    res.json(results);
  } catch (err) {
...
app.get('/api/leave-balances', authenticateToken, async (req, res) => {
  try {
    const [results] = await db.query(`
      SELECT e.id, e.ee_id, e.employee_name,
      ...
      FROM employees e
      LEFT JOIN leaves l ON e.id = l.employee_id AND YEAR(l.leave_date) = YEAR(CURDATE())
...
app.get('/api/leaves/monthly-report', authenticateToken, async (req, res) => {
  ...
  try {
    const [results] = await db.query(`
      SELECT e.ee_id AS 'EE ID', e.employee_name AS 'Employee Name',
        e.department AS 'Department', e.position_title AS 'Position',
        l.leave_date AS 'Leave Date', l.leave_type AS 'Leave Type'
      FROM employees e JOIN leaves l ON e.id = l.employee_id
...
app.get('/api/daily-attendance', authenticateToken, async (req, res) => {
  ...
  try {
    const [results] = await db.query(`
      SELECT e.ee_id AS 'Employee ID', e.employee_name AS 'Employee Name',
        e.department AS 'Department', l.leave_type AS 'Leave Status'
      FROM employees e
